...
def register_model():
    if not st.session_state.started:
        ...
        return
    url = "http://localhost:8081/models?model_name=" + MODEL_NAME + "&url=" + MODEL_NAME + ".tar.gz" + "&initial_workers=1&synchronous=true"
    _register_model(url)
...
def scale_workers(workers):
    if st.session_state.registered:
        ...
        url = "http://localhost:8081/models/" + MODEL_NAME + "?min_worker=" + str(num_workers) + "&synchronous=true"
...
def set_batch_size(batch_size):
    if st.session_state.registered:
        ...
        url = "http://localhost:8081/models?model_name=" + MODEL_NAME + "&url=" + MODEL_NAME + ".tar.gz" + "&batch_size=" + str(batch_size) + "&initial_workers=" + str(workers) + "&synchronous=true" + "&max_batch_delay=" + str(max_batch_delay)
        _register_model(url)
...
with st.sidebar:
    ...
    workers = st.sidebar.slider('Num Workers', key="Num Workers", min_value=1, max_value=4, value=1, step=1, on_change=scale_workers, args=("Num Workers",))
    batch_size = st.sidebar.select_slider('Batch Size', key="Batch Size", options=[2 ** j for j in range(0, 8)],  on_change=set_batch_size, args=("Batch Size",))
    max_batch_delay = st.sidebar.slider('Max Batch Delay', key="Max Batch Delay", min_value=100, max_value=10000, value=100, step=100,  on_change=set_max_batch_delay, args=("Max Batch Delay",))
