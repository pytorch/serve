...
async def read_from_s3():
    ...
    #Read tarf file from minio using s3 API
            ...
            for item in tar:
                ...
                if item.isfile() and "test" in item.name:
                    ...
                    with tar.extractfile(item) as f:
                        #Read each line from the file, take the comment which is 3rd column. Create a async task
                        ...
                        for row in f:
                            ...
                            if text:
                                if len(tasks) <= BATCH_TOTAL_TASKS:
                                    tasks.append(send_inference_request(aiohttp_client_session, text))
                                else:
                                    #Once we've TOTAL_TASKS send all inference request in short time fto allow
                                    #torchserver to do batch inference
                                    print(f"Sending {len(tasks)} inference calls. Total sent so far {TOTAL_INFERENCE_CALLS}.")
                                    result = await asyncio.gather(*tasks)
                                    if total_completed_tasks > TOTAL_INFERENCE_CALLS:
                                        print(f"Finished sending maximum number of inference calls {TOTAL_INFERENCE_CALLS}. Stopping...")
                                        break
                                    tasks = []
                                    tasks.append(send_inference_request(aiohttp_client_session, text))
