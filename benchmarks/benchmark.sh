...
response=$(curl --write-out %{http_code} --silent --output /dev/null --retry 5 http://localhost:8081/models/${MODEL})

if [ ! "$response" == 200 ]
then
    echo "Cannot run inference as this model is not registered with torchserve!!"
    exit 1
else
    echo "Model is already registered with torchserve.. Proceeding with apache benchmark performance tests now!!"
fi
