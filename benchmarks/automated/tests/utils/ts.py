...
class TorchServeHandler(object):
    def __init__(
        ...
        self.connection.run(
            f"pip3 install torch-model-archiver torch-workflow-archiver"
        )
    ...
    def setup_torchserve(self, virtual_env_name=None):
        ...
        self.connection.run(
            f"{activation_command}pip3 install torch-model-archiver torch-workflow-archiver"
        )
    ...
    def prepare_common_dependency(self):
        ...
        self.connection.run(
            f"mkdir -p {os.path.join(TMP_DIR, 'benchmark', 'wf_store')}"
        )
    ...
    def getAPIS(self):
        if self.is_local_execution:
            self.connection.run(
                f"cp {self.config_properties} {os.path.join(LOCAL_TMP_DIR, 'config.properties')}"
            )
        else:
            self.connection.get(
    ...
    def start_torchserve_local(self, virtual_env_name=None, stop_torchserve=True):
        ...
        LOGGER.info(
            f"{activation_command}torchserve --start --model-store /home/ubuntu/benchmark/model_store/ --workflow-store /home/ubuntu/benchmark/wf_store --ts-config {TMP_DIR}/benchmark/conf/config.properties --ncs > {TMP_DIR}/benchmark/logs/model_metrics.log"
        )
    ...
    def start_recording_docker_stats(self):
        ...
        self.connection.run(
            'docker exec -d ts bash -c "while true; do free |  grep -i mem | awk \'{ tmp=(\$3)/(\$2) ; printf\\"%0.2f\\n\\", tmp }\' >> /tmp/benchmark/logs/free.out; sleep 1 ; done "',
            echo=True,
            warn=True,
            pty=False,
        )
        self.connection.run(
            "nohup bash -c 'while true; do docker stats ts --format '{{.CPUPerc}}' --no-stream | sed 's/\%//g' 2>&1; sleep 0.5; done >& nohup.out < nohup.out & '",
            pty=False,
        )
        time.sleep(3)
    ...
    def stop_recording_docker_stats(self, model_name, num_workers, batch_size):
        ...
        self.connection.run(
            f"cp /home/ubuntu/benchmark/logs/free.out ./free.{model_name}.{num_workers}.{batch_size}",
            warn=True,
        )

        # Stops the nohup process that was recording stats
        self.connection.run(
            "ps axl|grep -e '--no-stream'| grep -v color | awk '{print $3}' | xargs kill -9",
            warn=True,
        )
        self.connection.run(
            f"cp nohup.out nohup.{model_name}.{num_workers}.{batch_size}", warn=True
        )
    ...
    def plot_stats_graph(self, model_name, mode_name, num_workers, batch_size):
        ...
        plt.title(
            f"{model_name} {mode_name} num_workers={num_workers} batch_size={batch_size}"
        )
        plt.savefig(
            f"free_plot.{model_name}.{mode_name}.{num_workers}.{batch_size}.png"
        )
        plt.clf()
    ...
    def unregister_model(self, model_name="benchmark"):
        ...
        """
        run_out = self.connection.run(
            f'curl -X DELETE "http://localhost:8081/models/{model_name}/"', warn=True
        )
    ...
    def register_workflow(self, url):
        ...
        """
        run_out = self.connection.run(
            f'curl -X POST "http://localhost:8081/workflows?url={url}.war"'
        )
    ...
    def unregister_workflow(self, workflow_name):
        ...
        """
        run_out = self.connection.run(
            f'curl -X DELETE "http://localhost:8081/workflows/{workflow_name}"'
        )
    ...
    def create_and_update_workflow_archive(
        self,
        workflow_name,
        spec_file_name,
        handler_file_name,
        ...
        self.connection.run(
            f"aws s3 cp {spec_file_template_path} {S3_BUCKET_BENCHMARK_ARTIFACTS}/{temp_uuid}/template.yaml"
        )
        time.sleep(2)
        run(
            f"aws s3 cp {S3_BUCKET_BENCHMARK_ARTIFACTS}/{temp_uuid}/template.yaml template.yaml"
        )
        time.sleep(2)
        ...
        spec_file_contents = re.sub(
            r"min-workers:(.*)", f"min-workers: {workers}", spec_file_contents
        )
        spec_file_contents = re.sub(
            r"max-workers:(.*)", f"max-workers: {workers}", spec_file_contents
        )
        spec_file_contents = re.sub(
            r"batch-size:(.*)", f"batch-size: {batch_size}", spec_file_contents
        )
        spec_file_contents = re.sub(
            r"max-batch-delay:(.*)",
            f"max-batch-delay: {batch_delay}",
            spec_file_contents,
        )
        spec_file_contents = re.sub(
            r"retry-attempts:(.*)",
        ...
        spec_file_contents = re.sub(
            r"timeout-ms:(.*)", f"timeout-ms: {timeout_ms}", spec_file_contents
        )
        ...
        run(
            f"aws s3 cp template.yaml {S3_BUCKET_BENCHMARK_ARTIFACTS}/{temp_uuid}/template.yaml"
        )
        time.sleep(2)
        self.connection.run(
            f"aws s3 cp {S3_BUCKET_BENCHMARK_ARTIFACTS}/{temp_uuid}/template.yaml {spec_file_template_path}"
        )
        ...
        self.connection.run(
            f"source activate python3 && pip install torch-workflow-archiver",
            shell=True,
        )
        run_out = self.connection.run(
            f"/home/ubuntu/.local/bin/torch-workflow-archiver -f --workflow-name {workflow_name} --spec-file {spec_file_template_path} --handler {handler_path} --export-path /home/ubuntu/benchmark/wf_store/",
            shell=True,
        )
        if run_out.return_code != 0:
        ...
        self.connection.run(
            f"cp -R {self.tmp_wf_dir}/*.mar /home/ubuntu/benchmark/model_store/",
            warn=True,
        )

        LOGGER.info(
            f"Updated workflow archive at location: /home/ubuntu/benchmark/wf_store/{workflow_name}.war"
        )

    def download_workflow_artifacts(
        self, workflow_name, model_urls, specfile_url, workflow_handler_url
    ):
        """
