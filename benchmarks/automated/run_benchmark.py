...
def build_docker_container(
    torchserve_branch="master", push_image=True, use_local_serve_folder=False
):
    ...
    if use_local_serve_folder:
        LOGGER.info(
            f"*** Using the local 'serve' folder closure when creating the container image."
        )
        ...
        run(
            f"rsync -av --progress {tmp_local_serve_folder}/ {serve_folder_in_docker_context}/"
        )
    ...
    for processor, config in docker_config.items():
        ...
        if not dockerhub_image:
            dockerImageHandler.build_image(
                use_local_serve_folder=use_local_serve_folder
            )
        else:
            # Image is pulled by process_docker_config in __init__.py
            LOGGER.info(
                f"*** Note: dockerhub_image specified in docker.yaml. This container image will be used for benchmark."
            )
        ...
        if push_image:
            ...
            LOGGER.warn(
                f"Docker image will not be pushed to ECR repo in local execution."
            )
...
def main():
    ...
    if arguments.local_execution and not arguments.local_instance_type:
        LOGGER.error(
            f"--local-instance-type must be specified when using --local-execution"
        )
        sys.exit(1)
    ...
    local_execution_string = (
        "" if not arguments.local_execution else "--local-execution"
    )
    ...
    if run_only_test:
        ...
        LOGGER.info(
            f"Note: running only the tests that have the name '{run_only_string_list}'."
        )
    else:
        run_only_string = ""
    ...
    if not arguments.skip_docker_build:
        ...
        build_docker_container(
            torchserve_branch=torchserve_branch,
