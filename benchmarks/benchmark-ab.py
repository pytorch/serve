...
def check_torchserve_health():
    ...
    while retry < attempts:
        try:
            ...
            time.sleep(3)
    failure_exit(
        "Could not connect to Tochserve instance at "
...
def warm_up():
    ...
    ab_cmd = (
        f"ab -c {execution_params['concurrency']}  -n {execution_params['requests']/10} -k -p {TMP_DIR}/benchmark/input -T "
...
def run_benchmark():
    ...
    ab_cmd = (
        f"ab -c {execution_params['concurrency']}  -n {execution_params['requests']} -k -p {TMP_DIR}/benchmark/input -T "
        f"{execution_params['content_type']} {execution_params['inference_url']}/{execution_params['inference_model_url']} > {result_file}"
    )
...
def local_torserve_start():
    ...
    execute(
        f"torchserve --start --model-store {TMP_DIR}/model_store "
...
def docker_torchserve_start():
    ...
    docker_run_cmd = (
        f"docker run {execution_params['docker_runtime']} {backend_profiling} --name ts --user root -p {inference_port}:{inference_port} -p {management_port}:{management_port} "
...
def prepare_common_dependency():
    ...
    shutil.copy(
        execution_params["config_properties"], os.path.join(TMP_DIR, "benchmark/conf/")
    )
...
def generate_latency_graph():
    ...
    df = pd.read_csv(
        os.path.join(TMP_DIR, "benchmark/predict.txt"), header=None, names=["latency"]
    )
    iteration = df.index
    latency = df.latency
    a4_dims = (11.7, 8.27)
