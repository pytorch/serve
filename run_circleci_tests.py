...
def create_processed_config(cci_config_file, processed_file):
    ...
    process_config_cmd = "circleci config process {} > {}".format(
        cci_config_file, processed_file
    )
...
def get_processed_job_sequence(procsd_jb_name, all_jbs_in_wrkflw):
    ...
    job_dict = next(
        (
            jd
            for jd in all_jbs_in_wrkflw
            if isinstance(jd, dict) and procsd_jb_name == list(jd)[0]
        ),
        None,
    )
    if job_dict:
...
def get_jobs_to_exec(job_name, all_jobs_in_wrkflw, executr, avlbl_executrs):
    ...
    for exectr_name in executors:
        if job_name is None:
            ...
            jobs_dict[exectr_name] = map(
                lambda j: j if isinstance(j, str) else list(j)[0], all_jobs_in_wrkflw
            )
            # Filter processed job names as per the executor
            # "job_name-executor_name" is a convention set in config.yml
            # pylint: disable=cell-var-from-loop
            jobs_dict[exectr_name] = filter(
                lambda j: exectr_name in j, jobs_dict[exectr_name]
            )
        else:
            # The list might contain duplicate parent jobs due to multiple fan-ins like config
            #     - Remove the duplicates
            # "job_name-executor_name" is a convention set in config.yml
            jobs_dict[exectr_name] = OrderedDict.fromkeys(
                get_processed_job_sequence(
                    job_name + "-" + exectr_name, all_jobs_in_wrkflw
                )
            )
...
def get_merged_jobs_steps(jobs, processed_cfg):
    ...
    for jname in jobs:
        ...
        merged_steps += list(
            filter(lambda step: list(step)[0] not in blocked_steps, steps)
        )
    return merged_steps
...
def execute_job(jbs_to_exec, processed_cfg, xformed_job_name, xformed_file):
    ...
    for exectr, jobs in jbs_to_exec.items():
        ...
        create_transformed_config(
            xformed_job, processed_cfg, xformed_job_name, xformed_file
        )
        ...
        try:
            ...
            local_execute_cmd = "circleci local execute -c {} --job {} \
                                ...
                                --env AWS_SECRET_ACCESS_KEY=`aws configure get aws_secret_access_key`".format(
                xformed_file, xformed_job_name
            )
...
def print_result(res):
    ...
    for exectr, retcode in res.items():
        colorcode, status = (
            ("\033[0;37;42m", "successful")
            if retcode == 0
            else ("\033[0;37;41m", "failed")
        )
        print(
            "{} Job execution {} using {} executor \x1b[0m".format(
                colorcode, status, exectr
            )
        )
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Execute circleci jobs in a \
    ...
    parser.add_argument(
        "-e", "--executor", type=str, help="Executor name from config.yml"
    )
    ...
    jobs_to_exec = get_jobs_to_exec(
        job, all_jobs_in_workflow, executor, available_executors
    )
