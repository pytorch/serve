...
class TestAddMetrics:
    def test_add_metric_passing(self):
        ...
        assert (
            "[gauge]-[new_metric]-[ModelName:my_tc,host:example_host_name,Level:Model]"
    ...
    def test_add_metric_duplicate_passing(self):
        ...
        assert (
            "[gauge]-[new_metric]-[ModelName:my_tc,Level:Model]"
            in metrics_cache_obj.cache
        )
...
class TestGetMetrics:
    ...
    def test_get_metric_dimensions_list_nonexist(self):
        ...
        assert (
            str(exc_info.value)
            == "'Metric key [gauge]-[new_metric]-[host:example_host_name,Level:Model] "
            "does not exist.'"
        )
...
class TestParseYaml:
    def test_parse_expected_yaml(self):
        ...
        expected_dict = {
            ...
            "ts_metrics": {
                "counter": [
                    {"name": {"dimensions": ["model_name", "host"], "unit": "ms"}}
                ],
                "gauge": [
                    {"name": {"dimensions": ["model_name", "host"], "unit": "ms"}}
                ],
    ...
    def test_yaml_file_non_yaml_extension_fail(self):
        with pytest.raises(merrors.MetricsCacheTypeError) as exc_info:
            MetricsCacheYaml(
                uuid.uuid4(), "ModelNameExample", "metric_cache_unit_test.py"
            )
        assert (
            str(exc_info.value) == "Input file metric_cache_unit_test.py "
...
class TestManualAddMetricDimensions:
    ...
    def test_dimensions_metric_dimension_not_present(self):
        metrics_cache_obj = MetricsCacheYaml(
            uuid.uuid4(), "ModelNameExample", "metrics_mismatching_dims.yaml"
        )
        with pytest.raises(merrors.MetricsCacheKeyError) as exc_info:
            metrics_cache_obj.parse_yaml_to_cache()
