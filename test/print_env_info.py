...
def get_conda_packages(run_lambda):
    grep_cmd = r'grep "torch\|numpy\|cudatoolkit"'
...
def get_pip_packages(run_lambda, package_name=None):
...
def get_env_info():
    ...
    _, torch_list_output = get_pip_packages(run_lambda,"torch")
    ...
    return SystemEnv(
        torch_packages=torch_list_output,
        ...
        java_packages=get_java_package(run_lambda),
...
def pretty_str(envinfo):
    def replace_nones(dct, replacement='N/A'):
        for key in dct.keys():
    ...
    mutable_dict['nvidia_gpu_models'] = \
        print_on_next_line(envinfo.nvidia_gpu_models)
    ...
    mutable_dict['torch_packages'] = replace_if_empty(mutable_dict['torch_packages'], "**Warning : Torchserve packages not found.")
    mutable_dict['java_packages'] = replace_if_empty(mutable_dict['java_packages'])
...
def get_java_package(run_lambda):
    rc, out, _ = run_lambda("java --version")
    if rc != 0:
