...
input_json_mnist = os.path.join(REPO_ROOT,
                                "kubernetes/kserve/kf_request_json/mnist.json")
...
def mnist_model_register_using_non_existent_handler_then_scale_up(
    ...
    """
    response = requests.post(TF_MANAGEMENT_API +
                             '/models?handler=nehandler&url=mnist.mar')
...
def run_inference_using_url_with_data_json(purl=None,
                                           json_input=None,
                                           ptimeout=120):
    if purl is None and pfiles is None:
        return
