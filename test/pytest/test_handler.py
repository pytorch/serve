...
def test_huggingface_opt_distributed_inference_pippy():
    TORCHSERVE_URL = "https://torchserve.s3.amazonaws.com/mar_files/opt-pippy.tar.gz"
    BATCH_SIZE = 1
    BATCH_DELAY = 10000  # 10 seconds
    INPUT_TEXT = os.path.join(
        REPO_ROOT, "examples", "large_models", "Huggingface_pippy", "sample_text.txt"
    )
    number_of_gpus = torch.cuda.device_count()
    ...
    if number_of_gpus > 1:
        try:
            ...
            assert (
                "Hey, are you conscious? Can you talk to me?" in response_text
