...
def setup_torchserve():
    if os.path.exists(TS_LOG):
        os.remove(TS_LOG)
    test_utils.start_torchserve(test_utils.MODEL_STORE, snapshot_file_ipex, gen_mar=False)
...
def get_worker_affinity(num_cores, num_workers, worker_idx):
    ...
    affinity += " -m {}".format(",".join([str(numa_id) for numa_id in cpuinfo.numa_aware_check(curr_worker_cores)]))
    return affinity

def run_inference_with_core_pinning():
    files = {
        'data': (data_file_kitten,
                 open(data_file_kitten, 'rb')),
    }
    response = run_inference_using_url_with_data('http://localhost:8080/predictions/resnet-18', files)
    return response 
...
def test_single_worker_affinity():
    ...
    worker_idx = 0 
    setup_torchserve()
    requests.post('http://localhost:8081/models?initial_workers={}&synchronous=true&url=resnet-18.mar'.format(num_workers))
    
    response = run_inference_with_core_pinning()
    assert response.status_code == 200, "single-worker inference with core pinning failed"
...
def test_multi_worker_affinity():
    ...
    response = run_inference_with_core_pinning()
    assert response.status_code == 200, "multi-worker inference with core pinning failed"
    
    for worker_idx in range(num_workers):
        curr_worker_affinity = get_worker_affinity(NUM_CORES, num_workers, worker_idx)
        assert curr_worker_affinity in open(TS_LOG).read(), "workers are not correctly pinned to cores"

def test_worker_scale_up_affinity():
    initial_num_workers = 2 
    setup_torchserve()
    requests.post('http://localhost:8081/models?initial_workers={}&synchronous=true&url=resnet-18.mar'.format(initial_num_workers))
    ...
    assert len(resnet18_list[0]['workers']) == scaled_up_num_workers, "workers failed to scale up with core pinning"
    
    response = run_inference_with_core_pinning()
    assert response.status_code == 200, "scaled up workers inference with core pinning failed"
    
    for worker_idx in range(scaled_up_num_workers):
        curr_worker_affinity = get_worker_affinity(NUM_CORES, scaled_up_num_workers, worker_idx)
        assert curr_worker_affinity in open(TS_LOG).read(), "workers are not correctly pinned to cores"
...
def test_worker_scale_down_affinity():
    ...
    resnet18_list = json.loads(response.content)
    assert len(resnet18_list[0]['workers']) == scaled_down_num_workers, "workers failed to scale down with core pinning"
    
    response = run_inference_with_core_pinning()
    assert response.status_code == 200, "scaled down workers inference with core pinning failed"
    
    for worker_idx in range(scaled_down_num_workers):
        curr_worker_affinity = get_worker_affinity(NUM_CORES, scaled_down_num_workers, worker_idx)
        assert curr_worker_affinity in open(TS_LOG).read(), "workers are not correctly pinned to cores"