...
def test_inference_stream_apis():
    with open(os.path.join(os.path.dirname(__file__), inference_stream_data_json), 'rb') as f:
        test_data = json.loads(f.read())
    ...
    for item in test_data:
        ...
        model_input = os.path.join(os.path.dirname(__file__), "..", item['file'])
        prediction = __infer_stream(test_gRPC_utils.get_inference_stub(), item['model_name'], model_input)
