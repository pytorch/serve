    ...
    private void testEndpoint() throws IOException {
        ...
        Assert.assertTrue(
                outputStream
                        .toString()
                        .matches(
                                "(?is).*ts_backend_reponse_latency_milliseconds_count.*model_name=\"noop\",model_version=\"1.0\",} 1.0.*"));
        Assert.assertTrue(
                outputStream
                        .toString()
                        .matches(
                                "(?is).*ts_backend_reponse_latency_milliseconds_sum.*model_name=\"noop\",model_version=\"1.0\",} 3.0.*"));
        Assert.assertTrue(
                outputStream
                        .toString()
                        .matches(
                                "(?is).*ts_inference_requests_total.*model_name=\"noop\",model_version=\"1.0\",} 1.0.*"));
        outputStream.reset();
        ...
        Assert.assertTrue(
                outputStream
                        .toString()
                        .matches(
                                "(?is).*ts_inference_requests_total.*model_name=\"noop\",model_version=\"1.0\",} 2.0.*"));
        outputStream.reset();
    }
}
