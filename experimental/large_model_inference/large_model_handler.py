...
class TransformersSeqClassifierHandler(BaseHandler, ABC):
    ...
    def initialize(self, ctx):
        ...
        dtype=torch.float32
        gpt_config = GPTLargeConfig(vocab_size=self.vocab_size, block_size=self.block_size)    
        model = sequential_gpt(gpt_config, devices = ["cuda:0", "cuda:1", "cuda:2", "cuda:3"], dtype=dtype)
        device_numel = [0, 0, 0, 0]
        for _, m in model.named_modules():
            for p in m.parameters():
                device_numel[p.device.index] += p.numel()
        assert max(device_numel) / min(device_numel) < 1.5
        self.model = Pipe(model, chunks=2)
    ...
    def preprocess(self, requests):
        ...
        x = torch.randint(0, self.vocab_size, (1, self.block_size)).cuda(0)
    ...
    def inference(self, input_batch):
        ...
        if self.setup_config["mode"] == "sequence_classification":
            predictions = self.model(input_ids_batch).local_value().sum()
