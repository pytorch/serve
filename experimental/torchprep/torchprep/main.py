...
def distill(
    ...
    parameter_scaling: int = 2,
    layer_scaling: int = None,
    ...
    print(f"Coming soon")
    print(
        "See this notebook for more information https://colab.research.google.com/drive/1RzQtprrHx8PokLQsFiQPAKzfn_DiTpDN?usp=sharing"
    )
...
def fuse(
    ...
    input_shape: str = typer.Option(
        default=None, help="Comma seperated input tensor shape e.g 64,3,7,7"
    ),
) -> torch.nn.Module:
    """
...
@app.command()
def env(
    device: Device = Device.cpu, omp_num_threads: int = 1, kmp_blocktime: int = 1
) -> None:
    """
...
def profile_model(
    model: torch.nn.Module, input_tensor, label: str = "model", iterations: int = 100
) -> List[float]:
