...
from torchprep.utils import ToyNet, load_model, profile_model
...
def test_prune():
    model_path = os.path.join(
        os.path.dirname(os.path.realpath(__file__)), "models", "resnet152.pt"
    )
...
def test_profile():
    ...
    result = profile_model(
        model=net,
        custom_profiler=Profiler.nothing,
        input_tensors=[torch.randn(10)],
        label="toy_profile",
        iterations=100,
    )
    assert len(result) == 3
...
def test_fuse():
    ...
    input_shape = os.path.join(
        os.path.dirname(os.path.realpath(__file__)), "config", "resnet.yaml"
    )
...
def test_multiple_input():
    config_file = os.path.join(
        os.path.dirname(os.path.realpath(__file__)), "config", "bert.yaml"
    )
    tensors = materialize_tensors(parse_input_format(config_file))
    model_path = os.path.join(
        os.path.dirname(os.path.realpath(__file__)), "models", "bert.pt"
    )
    model = load_model(model_path)
    profile_model(model, Profiler.nothing, tensors)
