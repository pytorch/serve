...
TorchServe exposes configurations that allow the user to configure the number of worker threads on CPU and GPUs. `number_of_gpu` determines the number of GPUs *per model* and should be set to `Number of GPUs / Number of Unique Models` to avoid under-utilization and GPU contention
