...
class CombinedModel(nn.Module):
    ...
    def _forward(self, batch_1: List[str], batch_2: List[str]) -> Tensor:
        ...
        tokens = F.to_tensor(tokens, padding_value=0).to(
            self._dummy_param.device
        )

        ids = [(l1+2)*[0,] + (l2+1)*[1,] for l1, l2 in zip(lengths_1, lengths_2)]
        masks = [len(i)*[1,] for i in ids]

        ids = F.to_tensor(ids, padding_value=0).to(
            self._dummy_param.device
        )

        masks = F.to_tensor(masks, padding_value=0).to(
            self._dummy_param.device
        )

        return torch.softmax(self._model(input_ids=tokens, token_type_ids=ids, attention_mask=masks)[0], 1)
    ...
    def forward(self, batch_1: Any, batch_2: Any) -> Tensor:
        ...
        if torch.jit.isinstance(batch_1, List[str]) and torch.jit.isinstance(batch_2, List[str]):
            return self._forward(batch_1, batch_2)
        else:
...
def main(args):
    ...
    if not pathlib.Path(LOCAL_FILE).exists():
        VOCAB_FILE = "https://huggingface.co/bert-base-cased-finetuned-mrpc/raw/main/vocab.txt"
    ...
    def run_model(model_, s0, s1):
        ...
        return "\n".join([f"{round(p[1] * 100)}% paraphrase" for p in paraphrase_results])

    bert = AutoModelForSequenceClassification.from_pretrained("bert-base-cased-finetuned-mrpc", torchscript=True)
    ...
    print(run_model(model, [sequence_0, sequence_0], [sequence_1, sequence_2]))
    print(run_model(model, [sequence_0], [sequence_2]))

    jit_model = torch.jit.script(model)
    print(run_model(jit_model, [sequence_0, sequence_0], [sequence_1, sequence_2]))
    torch.jit.save(jit_model, tokenizer_file)

    loaded_jit_model = torch.jit.load(tokenizer_file)
    print(run_model(loaded_jit_model, [sequence_0, sequence_1, sequence_2], [sequence_1, sequence_2, sequence_0]))
