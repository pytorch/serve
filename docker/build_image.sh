...
PYTHON_VERSION=3.9
DOCKER_BASE_IMAGE="pytorch/torchserve-base:latest"
...
if [ "${BUILD_TYPE}" == "ci" ] && ! $USE_CUSTOM_TAG ;
then
  DOCKER_TAG="pytorch/torchserve:ci-$MACHINE"
fi
...
DOCKER_BUILDKIT=1 docker build --file Dockerfile.base --build-arg BASE_IMAGE="${BASE_IMAGE}" --build-arg CUDA_VERSION="${CUDA_VERSION}"  --build-arg PYTHON_VERSION="${PYTHON_VERSION}" -t $DOCKER_BASE_IMAGE .
...
then
  DOCKER_BUILDKIT=1 docker build --file Dockerfile.ci  --build-arg MACHINE_TYPE="${MACHINE}" --build-arg PYTHON_VERSION="${PYTHON_VERSION}" --build-arg BRANCH_NAME="${BRANCH_NAME}" -t "${DOCKER_TAG}" .
else
